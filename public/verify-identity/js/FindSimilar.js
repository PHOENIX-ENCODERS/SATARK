module.exports = {
    ...
    getLargeFaceListId: async function getLargeFaceListid(sql, config) {
        var large_face_list_id = await require('./Get_Update_LargeFaceList_id.js')(sql, config); // Could cause issues
        console.log("LargeFaceListId: " + large_face_list_id);
        return large_face_list_id;
    },

    findSimilar: async function FindSimilar(source_img_url, FaceClient, sql, DatabaseConfig) {
        console.log("========FIND SIMILAR========");
        console.log();

        // const source_image_file_name = "https://criminalfacematch.blob.core.windows.net/face-input-data/input_img_1.png";

        var large_face_list_id = await module.exports.getLargeFaceListId(sql, DatabaseConfig); //Another place where it could cause issues

        let detected_face = await module.exports.detectFaceRecognize(source_img_url, FaceClient);
