module.exports = {
    ...
    findMatchingRecord: async function FindMatchingRecord(imgIds, inputDetails, sql, config) {
        ...
        if(imgIds.length != 0) {
            // console.log(imgIds);
            await module.exports.searchDBWithImgIds(sqlConnect, imgIds[0], criminalRecords, inputDetails);//Seems like Face APi returns Face id that matches the most(hence only 1 id)
    ...
    findSimilarFaces: async function FindSimilarFaces(inputImgUrl, client, sql, config, findSimilar) {
        let results = await findSimilar.findSimilar(inputImgUrl, client, sql, config);
